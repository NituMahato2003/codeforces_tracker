...
function TopSolution() {
    ...
    async function getSolutionLink(e) {
      ...
      try {
          ...
          if (user) {
              const res = await fetch(`https://codeforce-tracker.onrender.com/api/usertrack?user=${encodeURIComponent(user)}`);
              if (res.ok) {
    ...
    async function saveTopHandle() {
        ...
        try {
            const res = await fetch('https://codeforce-tracker.onrender.com/api/usertrack', {
                method: 'POST',
